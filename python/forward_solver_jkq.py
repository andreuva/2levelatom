...
for i in range(2,len(selected)):
    selected[i] = nodes_selected[i]
    if -5 < zz[i] < 5 and not selected[i-1] :
        selected[i] = 1
    elif -5 < zz[i] < 5:
        selected[i] = 0
...
#                   AUTHOR: ANDRES VICENTE AREVALO                          #
#############################################################################
def solve_profiles( a, r, eps, dep_col, Hd, Jm00_nodes, Jm20_nodes):
    """
    ...
    Jm20_shape = np.repeat(np.repeat(Jm20[ :, np.newaxis], len(wnorm), axis=1)[ :, :, np.newaxis], len(mus), axis=2)
    ...
    Jm20 = (3*mu_shape**2 - 1)*II + 3*(mu_shape**2 - 1)*QQ
    Jm20 = 1/np.sqrt(4**2 * 2) * np.sum(Jm20*weigths_shape, axis=-1)
    Jm20 = trapezoidal( phy_shape[:,:,0]*Jm20, wnorm)
    
    return II,QQ,Jm00[selected],Jm20[selected]