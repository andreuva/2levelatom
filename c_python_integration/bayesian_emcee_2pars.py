...
a_sol = 1e-2      #1e-5,1e-2 ,1e-4                # dumping Voigt profile a=gam/(2^1/2*sig)
r_sol = 1e-2      #1,1e-4   ,1e-10                 # XCI/XLI
eps_sol = 1e-3                          # Phot. dest. probability (LTE=1,NLTE=1e-4)
dep_col_sol = 1e-3             #0.1          # Depolirarization colisions (delta)
Hd_sol = .6                  #1          # Hanle depolarization factor [1/5, 1]
...
plt.savefig(f'../figures/chain_strechmove_{nsamples}_params_\
{a_sol}_{r_sol}_{eps_sol}_{dep_col_sol}_{Hd_sol}_weigths_{w_I}_{w_Q}.png')
...
plt.savefig(f'../figures/corner_strechmove_{nsamples}_params_\
{a_sol}_{r_sol}_{eps_sol}_{dep_col_sol}_{Hd_sol}_weigths_{w_I}_{w_Q}.png')
